...
void setup() {
  ...
  FuzzySet *Bmax = new FuzzySet(-50, -30, -20, -7);//13
  FuzzySet *Bmin = new FuzzySet(-20, -5, -5, -1);
  FuzzySet *z1 = new FuzzySet(-1, 0, 0, 1);
  FuzzySet *Fmin = new FuzzySet(1, 5, 5, 20);
  FuzzySet *Fmax = new FuzzySet(7, 20, 30, 50);
  
  angle->addFuzzySet(Bmax);
  angle->addFuzzySet(Bmin);
  angle->addFuzzySet(z1);
  angle->addFuzzySet(Fmin);
  angle->addFuzzySet(Fmax);
  
  fuzzy->addFuzzyInput(angle);
//////////////////////////////////////////////////////////
FuzzyInput *Dangle = new FuzzyInput(2);
  FuzzySet *DBmax = new FuzzySet(-70, -50, -35, -3);//13
//FuzzySet *Bmin = new FuzzySet(-8, -5, -5, -1);
FuzzySet *Dz1 = new FuzzySet(-3, 0, 0, 3);
//FuzzySet *Fmin = new FuzzySet(1, 5, 5, 8);
FuzzySet *DFmax = new FuzzySet(3, 35, 50, 70);
...
*/
  ...
  FuzzySet *Bmaximum = new FuzzySet(-200, -120, -120, -80);//-80
  FuzzySet *Bmedium = new FuzzySet(-80, -70, -70, -10);
  //FuzzySet *Bless = new FuzzySet(-40, -30, -30, -20);
  FuzzySet *nada = new FuzzySet(-10, 0, 0, 10);
  //FuzzySet *Fless = new FuzzySet(20, 30, 30, 40);
  FuzzySet *Fmedium = new FuzzySet(10, 70, 70, 80);
  FuzzySet *Fmaximum = new FuzzySet(80, 120, 120, 200);//1, 37, 120, 120
  
  
  speedOutput->addFuzzySet(Bmaximum);
  speedOutput->addFuzzySet(Bmedium);  
  speedOutput->addFuzzySet(nada);
  speedOutput->addFuzzySet(Fmedium);
...
//  
  ...
  FuzzyRuleAntecedent *IfFminAndDz1 = new FuzzyRuleAntecedent();
  //IfBmaxAndDBmax->joinSingle(z1);
  IfFminAndDz1->joinWithAND(Fmin,Dz1);

  FuzzyRuleConsequent *TheFminAndDz1 = new FuzzyRuleConsequent();
  TheFminAndDz1->addOutput(Fmedium);

  FuzzyRule *fuzzyRule10 = new FuzzyRule(10, IfFminAndDz1,TheFminAndDz1);
  fuzzy->addFuzzyRule(fuzzyRule10);

////////
  FuzzyRuleAntecedent *IfBminAndDz1 = new FuzzyRuleAntecedent();
  //IfBmaxAndDBmax->joinSingle(z1);
  IfBminAndDz1->joinWithAND(Bmin,Dz1);

  FuzzyRuleConsequent *TheBminAndDz1 = new FuzzyRuleConsequent();
  TheBminAndDz1->addOutput(Bmedium);

  FuzzyRule *fuzzyRule11 = new FuzzyRule(11, IfBminAndDz1,TheBminAndDz1);
...
void loop() {///
  ...
  U=constrain(U,-149,149);
  Serial.print("  ,, U: ");
  Serial.print(U);
  U=map(U,-150,150,-130,130);//90vel
