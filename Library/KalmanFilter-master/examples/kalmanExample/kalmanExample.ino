...
float x1_ant=0;
float x3_ant=0;
float k[4]={-0.8,-0.9,39,700};//-0.8,-0.9,30,150
int SetPoint=0;
float input;
...
void loop() {///
  ...
  Serial.print(",  Angle: ");
  Serial.print(int(getDistance(x3,chtime*1000000000)));
  
  x4=((getDistance(x3,chtime*1000000000)-x3_ant)/chtime);
  Serial.print(",  Vel-Angle: ");
  Serial.print(x4);
  Serial.print(",,     dt: ");
  Serial.print(chtime);
  
  U=-((k[0]*(SetPoint-x1))+(k[1]*x2)+(k[2]*getDistance(x3,chtime*1000000000))+(k[3]*x4));
  ...
  x1_ant=x1;
  x3_ant=getDistance(x3,chtime*1000000000);
}
